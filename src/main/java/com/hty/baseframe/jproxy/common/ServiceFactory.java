...
public class ServiceFactory {
        ...
        if (list.isEmpty()) {
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                if (cmp == 0 || cmp == 1) {
                    ...
                    logger.warn("LocalService: {} is already exists with condition: {}", localService.getClazz().getName(), localService.getConditions());
                    continue;
                } else if (cmp == 2) {
                    //新加入的LocalService条件包含当前LocalService，替换
                    logger.info("LocalService: {} replaced with condition: {}", localService.getClazz().getName(), localService.getConditions());
                    list.set(i, localService);
                } else {
                    logger.info("Add LocalService: {} with condition: {}", localService.getClazz().getName(), localService.getConditions());
                    list.add(localService);
                }
            }
    }
    ...
    public static synchronized void addRemoteService(RemoteService remoteService) {
        ...
        synchronized (list) {
            if (list.isEmpty()) {
                logger.info("Add RemoteService: {} with condition: {}", remoteService.getClazz().getName(), remoteService.getConditions());
                ...
                for (int i = 0; i < list.size(); i++) {
                    ...
                    if (cmp == 0 || cmp == 1) {
                        ...
                        logger.warn("RemoteService: {} is already exists with condition: {}", remoteService.getClazz().getName(), remoteService.getConditions());
                        continue;
                    } else if (cmp == 2) {
                        //新加入的RemoteService条件包含当前RemoteService，替换
                        logger.info("RemoteService [{}] replaced with condition: {}", remoteService, remoteService.getConditions());
     ...
     * @return
     */
    public static RemoteService getRemoteService(Class<?> type, String registryCenterId, Map<String, String> conditions) {
        ...
        for (RemoteService ls : remoteServices) {
            if (ConditionMatchUtil.isMatch(conditions, ls.getConditions())) {
                if (!StringUtil.isEmpty(registryCenterId)
                        && !StringUtil.equals(registryCenterId, ls.getCenterId())) {
                    continue;
                }
                return ls;
            }
